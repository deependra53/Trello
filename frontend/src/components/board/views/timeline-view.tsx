    ...
    <div className="flex h-full flex-col gap-3 p-4 pb-24 animate-fade-up lg:pb-4">
      <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-card p-3 shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-1">
          <Button variant="ghost" size="icon" onClick={() => setCursor(subMonths(cursor, 1))} aria-label="Previous month" className="h-8 w-8 rounded-lg">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={() => setCursor(new Date())}>
            Today
          </Button>
          <Button variant="ghost" size="icon" onClick={() => setCursor(addMonths(cursor, 1))} aria-label="Next month" className="h-8 w-8 rounded-lg">
            <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
      </div>

      <div className="flex-1 overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm">
        <div className="flex h-full">
          <div className="w-32 shrink-0 overflow-y-auto border-r border-border/60 bg-muted/30 scrollbar-thin sm:w-40 lg:w-48">
            <div className="sticky top-0 z-10 h-9 border-b border-border/60 bg-muted/60 px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              List
            </div>
              ...
              <div key={listId} className="border-b border-border/60">
                ...
                {cards.map((c) => (
                  <button
                    ...
                    className="block w-full truncate px-4 py-1.5 text-left text-xs text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground"
                  >
                    {c.title}
                  </button>
                ))}
                  ...
                  {cards.map((c) => {
                    ...
                    return (
                      <div key={c._id} className="relative h-[29px] border-b border-border/60">
                        <button
                          onClick={() => onOpenCard(c._id)}
                          className="absolute top-1.5 grid h-5 place-items-center rounded-md px-2 text-[10px] font-medium text-white shadow-sm transition-transform duration-150 hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
